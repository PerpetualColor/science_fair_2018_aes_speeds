...
void RotWord(unsigned char* n);
void AddRoundKey(int round, unsigned char* state, unsigned char key[]);
void SubBytes(unsigned char* state);
void ShiftRows(unsigned char* state);
void MixColumns(unsigned char* r);
static __inline__ unsigned long long current_instruction(void);
unsigned char* Cipher(unsigned char in[], unsigned char key[]);
...
int main() {
	...
	/* AES requires a different key for each round plus one for an initial process, requiring NUMROUNDS+1 keys, and each key requires BLOCKLEN 32-bit words, or BLOCKLEN*4 8-bit words (unsigned chars)*/
	unsigned char key_schedule[BLOCKLEN*(NUMROUNDS+1)*4];
...
void KeyExpansion(unsigned char key[KEYLEN*4], unsigned char key_schedule[BLOCKLEN*(NUMROUNDS+1)*4]) {
	int i, j;
	unsigned short temp;
	...
	/* iterate 32 bits at a time, each key is 128 bits */
	for (i = KEYLEN*4; i < BLOCKLEN*(NUMROUNDS+1)*4; i += 4) {
		key_schedule[i] = key_schedule[i-4];
		key_schedule[i+1] = key_schedule[i-3];
		key_schedule[i+2] = key_schedule[i-2];
		key_schedule[i+3] = key_schedule[i-1];
		/* KEYLEN*4 chars = 128 bits; the start of the next key */
		if (i % (KEYLEN*4) == 0) {
			RotWord(&key_schedule[i]);
			key_schedule[i] = getSBoxValue(key_schedule[i]);
			key_schedule[i+1] = getSBoxValue(key_schedule[i+1]);
			key_schedule[i+2] = getSBoxValue(key_schedule[i+2]);
			key_schedule[i+3] = getSBoxValue(key_schedule[i+3]);
			key_schedule[i] ^= rcon[(i/4)/KEYLEN];
		}
		key_schedule[i] ^= key_schedule[i-(KEYLEN*4)];
		key_schedule[i+1] ^= key_schedule[i+1-(KEYLEN*4)];
		key_schedule[i+2] ^= key_schedule[i+2-(KEYLEN*4)];
		key_schedule[i+3] ^= key_schedule[i+3-(KEYLEN*4)];
...
unsigned char* Cipher(unsigned char in[4*BLOCKLEN], unsigned char key[BLOCKLEN*(NUMROUNDS+1)*2]) {
	int i,j;
	unsigned char* state;
	int round;
	state = in;
	...
	SubBytes(state);
	ShiftRows(state);
	AddRoundKey(NUMROUNDS, state, key);


	return state;
}

void AddRoundKey(int round, unsigned char* state, unsigned char key[BLOCKLEN*(NUMROUNDS+1)*4]) {
	int i;
	for (i = 0; i < 16; i++) {
