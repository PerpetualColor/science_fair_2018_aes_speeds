...
void KeyExpansion(unsigned char key[], unsigned int key_schedule[]);
void RotWord(unsigned int* n);
void AddRoundKey(int round, unsigned int* state, unsigned int key[]);
void SubBytes(unsigned char* state);
void ShiftRows(unsigned char* state);
void MixColumns(unsigned char* r);
static __inline__ unsigned long long current_instruction(void);
unsigned char* Cipher(unsigned char in[], unsigned int key[]);
...
int main() {
	...
	unsigned int key_schedule[BLOCKLEN*(NUMROUNDS+1)];
...
void KeyExpansion(unsigned char key[KEYLEN*4], unsigned int key_schedule[BLOCKLEN*(NUMROUNDS+1)*4]) {
	int i, j;
	unsigned short temp;
	...
	for (i = KEYLEN; i < BLOCKLEN*(NUMROUNDS+1); i++) {
		...
		key_schedule[i] ^= key_schedule[i-KEYLEN];
...
unsigned char* Cipher(unsigned char in[4*BLOCKLEN], unsigned int key[BLOCKLEN*(NUMROUNDS+1)*2]) {
	int i,j;
	unsigned char* state;
	int round;
	state = in;

	AddRoundKey(0, (unsigned int*) state, key);
	...
	for (round = 1; round < NUMROUNDS; round++) {
		...
		AddRoundKey(round, (unsigned int*) state, key);
	}
	
	SubBytes(state);
	ShiftRows(state);
	AddRoundKey(NUMROUNDS, (unsigned int*) state, key);
	

	return state;
}

void AddRoundKey(int round, unsigned int* state, unsigned int key[BLOCKLEN*(NUMROUNDS+1)*4]) {
	int i;
	for (i = 0; i < 4; i++) {
		*(state+i) ^= key[4*round+i];
	}
}
